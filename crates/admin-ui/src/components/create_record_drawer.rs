...
pub fn create_record_drawer(props: &CreateRecordDrawerProps) -> Html {
    ...
    let relation_modal_html =
        if let Some((field_name, target_col, is_multi)) = (*active_relation_modal).clone() {
            ...
            let current_ids: Vec<String> = if is_multi {
                serde_json::from_str::<Vec<String>>(&current_field_val).unwrap_or_else(|_| {
                    current_field_val
                        .split(',')
                                                ...
                                                props.collection_fields.iter()
                                                    ...
                                                    if dt == "bool" {
                                                        ...
                                                        html! {
                                                         ...
                                                         if f.multiple {
                                                             let selected_ids: Vec<String> = serde_json::from_str::<Vec<String>>(&current_val)
                                                                 .unwrap_or_else(|_| {
                                                                     current_val
                                                                         .split(',')
                                                                         .map(|s| s.trim().to_string())
